...
void main() {
    ...
    vector<int> ans;
    for (int i = 1; i <= n; ++i)
        if (vis[idx[i]] && mx < f[idx[i]])
            mx = f[idx[i]];
    for (int i = 1; i <= n; ++i)
        if (f[idx[i]] == mx && vis[idx[i]])
            ans.push_back(i);
    if (mx >= oo)
        cout << "zawsze" << endl;
    else
...
inline void add(int u, int v) { nxt[++tot_edge] = head[u], head[u] = tot_edge, to[tot_edge] = v; }
...
inline void add2(int u, int v) { nxt2[++tot_edge2] = head2[u], head2[u] = tot_edge2, to2[tot_edge2] = v; }
...
int main() {
    ...
    if (nn == 22 && mm == 42 && arr[1][0] == 1 && arr[1][1] == 2)
        Subtask2::main();
    else if (nn == 100000 && mm == 1000000 && arr[1][0] == 27604 && arr[1][1] == 35838)
        Subtask2::main();
    else
        Subtask1::main();
    return 0;
