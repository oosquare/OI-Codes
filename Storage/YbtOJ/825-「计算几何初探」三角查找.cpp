...
public:
    ...
    long long area(const Vector &p) const { return abs((p - p1).cross(p2 - p1)); }
    ...
    bool operator<(const Segment &rhs) const { return slope() < rhs.slope(); }
...
int main() {
    ...
    for (int i = 1; i <= n; ++i)
        for (int j = 1; j < i; ++j) s[++tot] = Segment(p[i], p[j]);
