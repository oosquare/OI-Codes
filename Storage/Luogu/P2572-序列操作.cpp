...
void pushdown(node &now, node &lc, node &rc) {
    if (now.val != -1) {
        ...
        lc.allmax[now.val] = lc.lmax[now.val] = lc.rmax[now.val] =
            lc.r - lc.l + 1;
        rc.allmax[now.val] = rc.lmax[now.val] = rc.rmax[now.val] =
            rc.r - rc.l + 1;
        lc.allmax[now.val ^ 1] = lc.lmax[now.val ^ 1] = lc.rmax[now.val ^ 1] =
            0;
        rc.allmax[now.val ^ 1] = rc.lmax[now.val ^ 1] = rc.rmax[now.val ^ 1] =
            0;
...
void modify(int root, int type, int ml, int mr) {
    ...
    if (tree[root].l == ml && tree[root].r == mr) {
        if (type == 0 || type == 1) {
            ...
            tree[root].allmax[type] = tree[root].lmax[type] =
                tree[root].rmax[type] = tree[root].r - tree[root].l + 1;
            tree[root].allmax[type ^ 1] = tree[root].lmax[type ^ 1] =
                tree[root].rmax[type ^ 1] = 0;
            tree[root].val = type;
        } else {
    ...
    if (mid < ml)
        modify(root << 1 | 1, type, ml, mr);
    else if (mr <= mid)
        modify(root << 1, type, ml, mr);
    else
        modify(root << 1, type, ml, mid),
            modify(root << 1 | 1, type, mid + 1, mr);
...
int querysum(int root, int ql, int qr) {
    ...
    int mid = (tree[root].l + tree[root].r) / 2;
    if (mid < ql)
        return querysum(root << 1 | 1, ql, qr);
    else if (qr <= mid)
        return querysum(root << 1, ql, qr);
    else
        return querysum(root << 1, ql, mid) +
               querysum(root << 1 | 1, mid + 1, qr);
}
...
node querymax(int root, int ql, int qr) {
    ...
    int mid = (tree[root].l + tree[root].r) / 2;
    if (mid < ql)
        return querymax(root << 1 | 1, ql, qr);
    else if (qr <= mid)
        return querymax(root << 1, ql, qr);
    else {
        node res, lc = querymax(root << 1, ql, mid),
                  rc = querymax(root << 1 | 1, mid + 1, qr);
        pushup(res, lc, rc);
        return res;
    }
}
...
void build(int root, int l, int r) {
    ...
    if (l == r) {
        tree[root].sum = a[l];
        tree[root].allmax[0] = tree[root].lmax[0] = tree[root].rmax[0] =
            a[l] == 0;
        tree[root].allmax[1] = tree[root].lmax[1] = tree[root].rmax[1] =
            a[l] == 1;
        return;
    }
    int mid = (l + r) / 2;
