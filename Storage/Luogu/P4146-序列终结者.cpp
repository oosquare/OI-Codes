...
inline int getmax(int root) { return root ? tree[root].max : -0x7fffffff; }

inline void pushup(int root) {
    if (root) {
        tree[root].size = tree[ls(root)].size + tree[rs(root)].size + 1;
        tree[root].max =
            max(tree[root].key, max(getmax(ls(root)), getmax(rs(root))));
    }
}
...
inline int create(int key) {
    tree[++total] = (node){key, key, 0, 0, 0, 0, 1, myrand()};
    return total;
}
