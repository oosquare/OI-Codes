...
void build(int x, int l, int r) {
    ...
    tree[x].lans =
        max(tree[x << 1].lans, tree[x << 1].sum + tree[x << 1 | 1].lans);
    tree[x].rans =
        max(tree[x << 1 | 1].rans, tree[x << 1 | 1].sum + tree[x << 1].rans);
    tree[x].ans = max(max(tree[x << 1].ans, tree[x << 1 | 1].ans),
                      tree[x << 1].rans + tree[x << 1 | 1].lans);
}
