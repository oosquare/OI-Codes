...
inline void link(int x, int y) {
    edge[++edgeuuid] = (e){y, head[x]};
    head[x] = edgeuuid;
    edge[++edgeuuid] = (e){x, head[y]};
    head[y] = edgeuuid;
}
...
int query(int x, int y, int lca, int fa, int l, int r, int rk) {
    if (l == r)
        return l;
    int tmp = tree[ls(x)].sum + tree[ls(y)].sum - tree[ls(lca)].sum -
              tree[ls(fa)].sum;
    int mid = (l + r) >> 1;
    if (rk <= tmp)
...
void solve() {
    ...
    for (int i = 1; i <= n; ++i)
        mapping[i] = lower_bound(data.begin(), data.end(), mapping[i]) -
                     data.begin() + 1;
    for (int i = 1; i <= n; ++i) {
        if (!vis[i]) {
            DFS(i, 0, i);
    ...
    while (t--) {
        ...
        if (cmd[0] == 'Q') {
            ...
            k ^= lastans;
            int lca = LCA(x, y);
            lastans = data[query(root[x], root[y], root[lca], root[f[lca][0]],
                                 1, n, k) -
                           1];
            printf("%d\n", lastans);
        } else {
