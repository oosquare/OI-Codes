...
void assign(int x, int l, int r, int ml, int mr, int v) {
    ...
    int mid = (l + r) >> 1;
    if (ml <= mid)
        assign(x << 1, l, mid, ml, mr, v);
    if (mid < mr)
        assign(x << 1 | 1, mid + 1, r, ml, mr, v);
    pushup(x);
}
...
int query(int x, int l, int r, int ql, int qr, int v) {
    ...
    int res = 0;
    if (ql <= mid)
        res += query(x << 1, l, mid, ql, qr, v);
    if (mid < qr)
        res += query(x << 1 | 1, mid + 1, r, ql, qr, v);
    return res;
}
...
node queryall(int x, int l, int r, int ql, int qr) {
    ...
    int mid = (l + r) >> 1;
    if (qr <= mid)
        return queryall(x << 1, l, mid, ql, qr);
    else if (mid < ql)
        return queryall(x << 1 | 1, mid + 1, r, ql, qr);
    else {
        node res1 = queryall(x << 1, l, mid, ql, qr),
             res2 = queryall(x << 1 | 1, mid + 1, r, ql, qr);
        for (int i = 0; i < 26; ++i) {
