...
int bfs() {
    ...
    for (int i = 0; i < MAX_N * 6; i++) graph[i].clear();
...
struct Vector {
    ...
    double dot(const Vector& b) const { return x * b.x + y * b.y; }

    double cross(const Vector& b) const { return x * b.y - y * b.x; }

    Vector rotate() const { return Vector(-y, x); }

    bool operator==(const Vector& b) const { return abs(b.y - y) < EPSLION && abs(b.x - x) < EPSLION; }

    bool same(const Vector& rhs) const { return abs(this->cross(rhs)) < EPSLION; }
};
...
struct Line {
    ...
    void init(const Vector& x, const Vector& y) { a = x, b = y, d = b - a, alpha = atan2(d.y, d.x); }
    ...
    Line(const Vector& a, const Vector& b) { init(a, b); }

    bool contain(const Vector& v) const { return d.same(v - a) && (a - v).dot(v - b) < EPSLION; }

    bool parallel(const Line& rhs) const { return d.same(rhs.d); }

    bool operator<(const Line& rhs) const { return alpha < rhs.alpha; }

    Vector intersection(const Line& rhs) const {
        return a + d * ((rhs.d.cross(a - rhs.a)) / (d.cross(rhs.d)));
    }

    bool right(const Vector& rhs) const { return ((rhs - a).cross(d)) > -EPSLION; }
...
int get(Line* a, int n, Vector* res, int& l, int& r, int st, const Vector& now) {
    ...
    for (int i = 2; i <= n; i++) {
        while (l < r && a[i].right(res[r])) --r;

        while (l < r && a[i].right(res[l + 1])) ++l;
    ...
    for (int i = l; i <= r; i++) Graph::link(st, q[i].id);
...
int main() {
    ...
    while (t--) {
        ...
        for (int i = 1; i <= n; i++) cin >> Geometry::a[i];
