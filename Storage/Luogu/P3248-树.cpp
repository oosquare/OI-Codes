/**
 * 查询结点 x 的深度：
 * 先找到 x 的子树的根节点在原树中的对应位置 rt,设 x 在其子树内的排名为 k，子树在第 i 次被添加，则在主席树上二分找到对应结点 x1 ，查询版本 i 的 dfn[x1] 位置的值。
 * 添加子树 (fr, to), 设是第 i 次：
 * 从版本 1 复制出版本 i + 1，查询 to 的深度，在版本 i + 1 的区间 [dfn[fr], dfn[fr] + size[fr] - 1] 上加上 to 的深度。
 */
