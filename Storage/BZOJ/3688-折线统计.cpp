...
struct BinaryIndexedTree {
    ...
    inline void modify(int x, int y) {
        for (; x <= maxn; x += (-x & x)) {
            Array[x] = ((long long)Array[x] + y) % mod;
        }
    }
...
int main() {
    scanf("%d%d", &n, &k);
    for (int i = 1; i <= n; ++i) {
        scanf("%d%d", &tmppos[i].X, &tmppos[i].Y);
    }
    sort(tmppos + 1, tmppos + 1 + n);
    for (int i = 1; i <= n; ++i) {
        posy[i] = tmppos[i].Y;
    }
    up[0].modify(posy[1], 1);
    down[0].modify(posy[1], 1);
    for (int i = 2; i <= n; ++i) {
        f[i][0][0] = f[i][0][1] = 1;
        for (int j = 1; j <= k; ++j) {
            f[i][j][0] =
                (up[j].rquery(1, posy[i]) + down[j - 1].rquery(1, posy[i])) %
                mod;
            f[i][j][1] = (down[j].rquery(posy[i] + 1, maxn) +
                          up[j - 1].rquery(posy[i] + 1, maxn)) %
                         mod;
        }
        for (int j = 0; j <= k; ++j) {
