...
edge tree[2*maxn]; int head[maxn],tot;
int f[maxn],sze[maxn],vis[maxn],cnt[maxn],dis[maxn],tmp[maxn];
int root,sum,timer,n,ans,k;

void link(int x,int y,int w) {
    tree[++tot]=(edge){y,head[x],w}; head[x]=tot;
    tree[++tot]=(edge){x,head[y],w}; head[y]=tot;
}
...
void update(int x,int fa,int type) {
    if(dis[x]<=k) {
        if(type==1) tmp[dis[x]]=min(tmp[dis[x]],cnt[x]);
        else tmp[dis[x]]=0x3f3f3f3f;
    }
    for(int i=head[x],y;y=tree[i].to,i;i=tree[i].nxt) {
        if(vis[y]||y==fa) continue;
        update(y,x,type);
...
int main() {
    ...
    sum=n;
    f[0]=n;
    for(int i=1;i<=k;++i) tmp[i]=0x3f3f3f3f;
    getroot(1,0);
    solve(root);
    if(ans!=0x3f3f3f3f)
        printf("%d\n",ans);
