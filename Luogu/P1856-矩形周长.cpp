...
void pushup(int root) {
    if(tree[root].cover) tree[root].len=tree[root].r-tree[root].l;
    else tree[root].len=tree[root<<1].len+tree[root<<1|1].len;
}

void build(int root,int l,int r) {
    tree[root].l=v[l]; tree[root].r=v[r]; tree[root].cover=0;
    if(r-l<=1) return;
    int mid=(l+r)>>1;
    build(root<<1,l,mid);
...
void modify(int root,int ml,int mr,int val) {
    ...
    if(ml<tree[root<<1].r) modify(root<<1,ml,mr,val);
    if(mr>tree[root<<1|1].l) modify(root<<1|1,ml,mr,val);
    pushup(root);
}
...
int calcUnion(int l1,int r1,int l2,int r2) {
    if(l1<=l2) {
        if(r1<=r2) return max(0,r1-l2);
        else return r2-l2;
    } else {
        if(r2<=r1) return max(0,r2-l1);
        else return r1-l1;
    }
}
...
int calcx() {
    ...
    for(int i=2;i<=segtot;++i) {
        if(seg[i].a==seg[i-1].a) {
            expect+=2*calcUnion(seg[i-1].b,seg[i-1].c,seg[i].b,seg[i].c);
            //printf("(%d %d %d %d %d)",seg[i-1].b,seg[i-1].c,seg[i].b,seg[i].c,2*calcUnion(seg[i-1].b,seg[i-1].c,seg[i].b,seg[i].c));
        }
