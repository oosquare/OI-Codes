...
inline int myrandom() { return seed=(int)((seed+3)*341241LL%0x7fffffff); }

inline int create(int v){ tree[++total]={v,0,1,0,0,1,myrandom()}; return total; }

inline void pushup(int x) { tree[x].size=tree[ls(x)].size+tree[rs(x)].size+1; }
...
inline void pushdown(int x){
    if(tree[x].mul!=1){
        int xm=tree[x].mul; tree[x].mul=1;
        if(ls(x)){
    ...
    if(tree[x].add){
        int am=tree[x].add; tree[x].add=0;
        if(ls(x)){
...
void split(int root,int sze,int &x,int &y){
    ...
    if(tree[ls(root)].size<sze){
        x=root; split(rs(x),sze-tree[ls(x)].size-1,rs(x),y); pushup(x);
    }else{
        y=root; split(ls(y),sze,x,ls(y)); pushup(y);
    }
}
...
int main(){
    ...
    for(int i=1;i<=n;++i){
        scanf("%s",cmd);
        if(cmd[0]=='a') add();
        else if(cmd[0]=='m'&&cmd[3]!='x') mul();
        else if(cmd[0]=='m'&&cmd[3]=='x') mulx();
        else query();
    }
    return 0;
}
