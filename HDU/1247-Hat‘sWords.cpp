...
struct trie
{
    ...
    void insert(char str[]){
        ...
        for(int i=len;i>=1;--i){
            int c=str[i]-'a';
            if(!treeReserve[it][c]) treeReserve[it][c]=++totalReserve;
            it=treeReserve[it][c];
        }
        endReserve[it]=true;
    }
    ...
    bool findReserve(char str[]){
        ...
        for(int i=len;i>=1;--i){
            int c=str[i]-'a';
            if(!treeReserve[it][c]) return false;
            it=treeReserve[it][c];
            if(endReserve[it]) return true;
        }
        return false;
    }
